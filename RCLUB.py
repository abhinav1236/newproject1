...
main_application.title('RCLUB TEXT EDITOR')
...
select_icon=tk.PhotoImage(file='icons2/select.png')
        ...
        if text_changed:
            ...
            if mbox is True:
                if url:
                    ...
                    url=filedialog.assaveasfile(mode = 'w',defaultextension='.text',filetypes=(('Text File','*.txt'),('All Files','*.*')))
                    url.write(content2)
                    url.close()
                    main_application.destroy()
            elif mbox is False:
...
edit.add_command(label='Select All' , image=select_icon, compound=tk.LEFT , accelerator='Ctrl+A' , command= lambda:text_editor.event_generate("<Control a>"))
